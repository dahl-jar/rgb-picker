...
void RGBController::SetActiveMode(int mode)
{
    ...
    {
        for(unsigned int zone_idx = 0; zone_idx < (unsigned int)zones.size(); zone_idx++)
        {
...
unsigned char * RGBController::GetZoneDescriptionData(unsigned char* data_ptr, zone zone, unsigned int protocol_version)
{
    ...
    if((zone.flags & ZONE_FLAG_MANUALLY_CONFIGURABLE_SIZE_EFFECTS_ONLY) && (protocol_version < 5))
    {
        ...
        | value of either 0 or 1 depending on LED count     |
        \*-------------------------------------------------*/
        unsigned int tmp_size = (zone.leds_count > 0) ? 1 : 0;
...
unsigned char* RGBController::SetDeviceDescription(unsigned char* data_ptr, unsigned int data_size, RGBController* controller, unsigned int protocol_version)
{
    ...
    {
        data_ptr = SetModeDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &controller->modes[mode_index], protocol_version);
    ...
    {
        data_ptr = SetZoneDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &controller->zones[zone_index], protocol_version);
    ...
    {
        data_ptr = SetLEDDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &controller->leds[led_index], protocol_version);
    ...
    | Copy in colors                                        |
    \*-----------------------------------------------------*/
    data_ptr = SetColorDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), controller, protocol_version, true);
...
unsigned char* RGBController::SetSegmentDescription(unsigned char* data_ptr, unsigned int data_size, segment* segment, unsigned int protocol_version)
{
    ...
    if(protocol_version >= 6)
    {
        ...
        if(matrix_map_len > 0)
        {
            data_ptr = SetMatrixMapDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &segment->matrix_map, protocol_version);
...
unsigned char* RGBController::SetZoneDescription(unsigned char* data_ptr, unsigned int data_size, zone* zone, unsigned int protocol_version)
{
    ...
    if(matrix_map_len > 0)
    {
        data_ptr = SetMatrixMapDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &zone->matrix_map, protocol_version);
    ...
    if(protocol_version >= 4)
    {
        ...
        {
            data_ptr = SetSegmentDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &zone->segments[segment_index], protocol_version);
    ...
    if(protocol_version >= 6)
    {
        ...
        for(unsigned short mode_index = 0; mode_index < num_modes; mode_index++)
        {
            data_ptr = SetModeDescription(data_ptr, data_size - (unsigned int)(data_ptr - data_start), &zone->modes[mode_index], protocol_version);
...
bool RGBController::CompareControllers(RGBController* controller_1, RGBController* controller_2)
{
    /*-----------------------------------------------------*\
    ...
    {
        for(unsigned int zone_index = 0; zone_index < (unsigned int)controller_1->zones.size(); zone_index++)
        {
